...
For å kjøre et bash-script må en stå i samme mappe som scriptet i terminalen og skrive:
```bash
chmod +x <navn>.sh #chmod (change mode) +x gir bruker executerettigheter
./<navn>.sh #./ trengs for å spesifisere mappa du står i
```

Alternativt kan en skrive: `bash <navn>.sh`

Hvis et script skal være kjørbart fra hvor som helst i systemet, må det legges i `usr/local/bin`. Eksempel: Har du et script som heter `<navn>.sh` og legger fila i `usr/local/bin` kan hvem som helst kjøre det ved å skrive `<navn>` i terminalen.
...
#!/usr/bin/env bash
```
Dette forteller programmet hvor `PATH` til Bash er.

Etter dette bør den neste linja være
```bash
set -euo pipefail
# -e: stopp ved feil
# -u: stopp ved bruk av udefinert variabel
# -o pipefail: pipeline feiler hvis ett ledd feiler
```
...
Legg merke til at `sammenligningsoperatorene` er annerledes i Bash enn i mange andre språk. Hvis du vil [lære mer om operatorer i Bash, sjekk ut denne sida.](https://www.w3schools.com/bash/bash_operators.php)
...
```
OBS! Scriptet ovenfor er ikke testet på server ennå